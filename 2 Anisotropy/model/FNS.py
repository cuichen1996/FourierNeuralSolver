...
from model.unet import MetaUNetv2

class Anisotropy(nn.Module):
    def __init__(self, kernel):
        super(Anisotropy, self).__init__()
        self.kernel = kernel #[B, 1, 3, 3] bilinear element stencil for anisotropic equation on structured grid
    ...
    def fns(self, x, f, H, weights, smoother):
        if smoother == "jacobi":
            x = self.jacobi(x, f, w=1/2, M=10) #* M times
        elif smoother == "chebyshev":
            x = self.chebysemi(x, f, alpha=3, M=10)
        elif smoother == "cnn":
        ...
        alpha = self.compute_lr(r, e)
        x = x + alpha*e
        return x

    def compute_lr(self, r, e):
        Ae = self.forward(e)
        alpha = torch.inner(r.flatten(1), e.flatten(1)).diag() / torch.inner(Ae.flatten(1), e.flatten(1)).diag() 
...
class HyperFNS(nn.Module):
    ...
    def setup(self, kernelA, N):
        ...
        if self.smoother == "jacobi":
            ...
            if self.Meta_H_type == "sFNO":
                gridx, gridy = get_grid2D(lfa.shape, lfa.device)
                no_input = torch.cat((lfa, gridx, gridy), 1)
            else:
                no_input = lfa
            theta = self.meta(no_input.float(), 2*(N+1)) * LFA
    ...
    def ComputeSmootherFactor(self, KernelA, N, w=1/2):
    ...
    def fcg(self, f, kernelA, epoch):
        ...
        def B(r):
            ...
            return A.fns(x, r, self.H, weights, self.smoother)
